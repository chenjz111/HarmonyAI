...
| --- | --- | --- |
| `knowledge/v3/document-relevance-rules-v3.1-candidate.json` | Document Relevance 医学规则（VALID/INVALID/IRRELEVANT/INSUFFICIENT + reason-code 白名单 + 判定决策表 + 硬性不变量） | `deb74a7b…d7c0` |
| `knowledge/v3/usergoal-vocabulary-v3.1-candidate.json` | UserGoal 正式词表审核稿（7 code 非医学语义 + 证据边界 UG-01~06 + 消费契约） | `6df53cd7…632c` |
| `knowledge/v3/five-tone-safe-expression-rules-v3.1-candidate.json` | “五音调适解析”安全医学表达规则（FT-01~08 + 区块规则 + 免责声明候选文案） | `ec846256…8e448` |
...
| 明确 UserGoal 不属于 Medical Evidence | usergoal-vocabulary §evidence_boundary_rules（UG-01~06）+ §audit_conclusion | ✅ 候选稿 |
| 制定“五音调适解析”安全医学表达规则 | five-tone-safe-expression-rules（FT-01~08 + section_rules + disclaimer） | ✅ 候选稿 |
| 继续维护现有 RAG 医学资产 | 见 §5；本批未改动任何既有 v3.0 资产 | ✅ 只读维护 |

## 3. 范围外确认（遵守 #97 边界）

- **未改动 Q1~Q10**：questionnaire-v3.0.json 与既有 manifest 零改动（Gate #4 checksum 证据仍待冻结时核验）。
- **未改前端**：未触碰任何 .vue / read-model / frontend 文件。
- **未写 Agent 代码**：本批全部为医学资产（JSON 规则 + 文档），不含实现。
- **未 merge / 未开 PR**：仅推送到个人 feature branch（`feat/s5-v3.1-medical-relevance-rules`），遵守 V3.1 PROVISIONAL 铁律与用户“不开 PR”指示。
...
4. **表达安全**：在既有 manifest forbidden_expressions 基础上扩展确诊式/疗效承诺/脏腑结论式禁令（FT-01~03），并要求后端安全装配 + 前端只读（FT-07），免责声明给出候选文案（DIS-V3.1-01，待老师审阅）。
...
3. 冻结后将 3 资产注册入 knowledge-manifest（checksum 三方对齐），并核验 Gate #4（Q1~Q10 不变）；
