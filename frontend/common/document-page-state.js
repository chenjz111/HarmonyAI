...
export function applyOcrResponse(current, response = {}) {
  ...
  if (status === 'failed') {
    const errorCode = response.error_code || 'OCR_FAILED'
    return { ...current, mode: 'failed', text: '', errorCode, message: response.message || SAFE_OCR_MESSAGES[errorCode] || SAFE_OCR_MESSAGES.OCR_FAILED, editorVisible: false, canConfirm: false, navigateAssessment: false }
  }
  if (status === 'degraded') {
    return { ...current, mode: 'degraded', text: response.extracted_text || '', errorCode: response.error_code || '', message: response.message || 'OCR 仅识别出部分内容，请核对后确认。', editorVisible: true, canConfirm: true, navigateAssessment: false }
  }
